...
export async function POST(request) {
  ...
  try {  
    ...
    if (body.subject === "An Order from Two4line website") {
    ...
    } else if (body.subject === "A Client Message") {
      await transporter.sendMail({ 
        from: "two4line@gmail.com",
        to: "two4line@gmail.com",
        subject: "A Client Message",
        text: "A Client Message",
        html: `<div style="font-family: Arial, sans-serif;">
        <h2 style="font-size: 24px; margin-bottom: 20px;">Order Details</h2>
        <div style="margin-bottom: 20px;">
          <h4 style="font-size: 18px; margin-bottom: 5px;">Name:</h4>
        ...
        <div style="margin-bottom: 20px;">
          <h4 style="font-size: 18px; margin-bottom: 5px;">Client Email:</h4>
          <div style="font-size: 16px;">${body.email}</div>
        </div>
        <div style="margin-bottom: 20px;">
          <h4 style="font-size: 18px; margin-bottom: 5px;">Message:</h4>
          <div style="font-size: 16px;">${body.message}</div>
        </div>
      </div>
    `
      });
